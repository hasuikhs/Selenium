...
new_id_set = set(file_df['id'])
### 7.searching...
main_index = 0
index = 0
try:
  while len(new_id_set) < LIMIT_CNT:
    time.sleep(1)
    ...
    if follower >= LIMIT_FOLLOWER and id not in id_set and id not in new_id_set and 'official' not in id:
      new_id_set.add(id)
      index = index + 1
      df.loc[len(df)] = [index, id, f'https://www.instagram.com/{ id }', follower]
